...
setwd("/Volumes/GEOG331_F25/data/oneida")
...
#view the matrix
rf_errorM$overall
#producer_accuracy <- diag(rf_errorM) / colSums(rf_errorM)

by_class_metrics <- rf_errorM$byClass
user_accuracy_per_class <- by_class_metrics[, "Precision"]
producer_accuracy_per_class <- by_class_metrics[, "Recall"]
print("User's Accuracy (Precision):")
print(user_accuracy_per_class)
print("Producer's Accuracy (Recall):")
print(producer_accuracy_per_class)

overall_accuracy <- rf_errorM$overall["Accuracy"]
print(paste("Overall Accuracy:", round(overall_accuracy, 4)))


# Example confusion matrix
confusion_matrix <- matrix(c(31, 0, 7, 2, 5, 0,
                             0, 47, 2, 0, 0, 1,
                             0, 7, 39, 0, 4, 0,
                             0, 0, 0, 40, 9, 0,
                             0, 0, 1, 2, 47, 0,
                             0, 0, 0, 0, 0, 49),
                           nrow = 6, byrow = TRUE)
colnames(confusion_matrix) <- c("Class1", "Class2", "Class3", "Class4", "Class5", "Class6")
rownames(confusion_matrix) <- c("Class1", "Class2", "Class3", "Class4", "Class5", "Class6")
user_accuracy <- diag(confusion_matrix) / rowSums(confusion_matrix)
producer_accuracy <- diag(confusion_matrix) / colSums(confusion_matrix)
